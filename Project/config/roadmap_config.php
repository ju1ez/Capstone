...
// If user clicked on a plan.. (on welcome.php)
if($_SERVER['REQUEST_METHOD'] === "GET" && isset($_GET["plan"])) {
  $_SESSION["plan_num"] = $_GET["plan"];
  // clicked on a plan that they had no save for, load template
  if(!isset($_SESSION["plans"][$_GET["plan"]])) {
  ...
  // clicked on a plan they had a save for
  } else {
